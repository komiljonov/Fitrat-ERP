...
class DashboardView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        if teacher:
            ...
            first_lesson_come = first_lesson_come.filter(lids_group__group__teacher__id=teacher)
            first_lesson_come_archived = first_lesson_come_archived.filter(lids_group__group__teacher__id=teacher)
            first_course_payment = first_course_payment.filter(lids_group__group__teacher__id=teacher)
            first_course_payment_archived = first_course_payment_archived.filter(lids_group__group__teacher__id=teacher)
        ...
        if course:
            ...
            first_lesson = first_lesson.filter(lids_group__group__course__id=course)
            first_lesson_come = first_lesson_come.filter(lids_group__group__course__id=course)
            first_lesson_come_archived = first_lesson_come_archived.filter(lids_group__group__course__id=course)
            first_course_payment = first_course_payment.filter(lids_group__group__course__id=course)
            first_course_payment_archived = first_course_payment_archived.filter(lids_group__group__course__id=course)
