...
class LeadSerializer(serializers.ModelSerializer):
    ...
    call_operator = serializers.PrimaryKeyRelatedField(
        queryset=CustomUser.objects.all(),
        allow_null=True,
    )
    service_manager = serializers.PrimaryKeyRelatedField(
        queryset=CustomUser.objects.all(),
        allow_null=True,
    )
    ...
    def get_filtered_queryset(self):
        ...
        if user.role == "CALL_OPERATOR":
            queryset = queryset.filter(
                Q(call_operator=user) | Q(call_operator=None), filial=None
            )
        return queryset
    ...
    def get_course(self, obj):
        return list(
            StudentGroup.objects.filter(lid=obj).values_list(
                "group__course__name", "group__course__level__id"
            )
        )
    ...
    def update(self, instance, validated_data):
        ...
        for attr, value in validated_data.items():
            if attr not in [
                "file",
                "call_operator",
            ]:  # Skip file field to avoid conflict
    ...
    def create(self, validated_data):
        ...
        files = validated_data.pop(
            "file", None
        )  # Remove the file field from validated_data
    ...
    def update_bulk_lids(self, lids_data):
        ...
        for lid_data in lids_data:
            ...
            try:
                ...
                lid_instance = self.update(
                    lid_instance, lid_data
                )  # This uses the `update` method from ModelSerializer
