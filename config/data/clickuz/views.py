...
class ClickUzMerchantAPIView(APIView):
    ...
    def complete(self, click_trans_id, amount, error, merchant_prepare_id,
        ...
        """
        from .status import A_LACK_OF_MONEY, A_LACK_OF_MONEY_CODE, INVALID_AMOUNT, INVALID_ACTION, TRANSACTION_NOT_FOUND
        ...
        try:
            ...
            if error == A_LACK_OF_MONEY:
                self.VALIDATE_CLASS().cancel_payment(transaction.merchant_trans_id, transaction)
            ...
            if transaction.action == action or transaction.status == Transaction.FINISHED:
