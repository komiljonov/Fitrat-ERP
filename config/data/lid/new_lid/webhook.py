...
class LidWebhook(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if not marketing_channel:
            return Response({'error': 'Marketing channel not found'},
                            status=status.HTTP_400_BAD_REQUEST)
        if data.get("subject") :
            subject_name = data.get("subject", "").strip().lower()
            all_subjects = Subject.objects.all()

            best_match = difflib.get_close_matches(subject_name, [s.name.lower() for s in all_subjects], n=1,
                                                   cutoff=0.5)

            subject = None
            if best_match:
                subject = next((s for s in all_subjects if s.name.lower()
                                == best_match[0]), None)
        ...
        if data.get("parents") :
            parent = Relatives.objects.create(
        ...
        return Response({'status': 'Thank youu mann!'},
