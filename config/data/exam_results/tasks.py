...
def send_unit_test_notification(unit_test_id, group_id):
    unit_test = (
        UnitTest.objects.filter(id=unit_test_id).prefetch_related("themes").first()
    )
    if not unit_test:
        return
    ...
    group_students = StudentGroup.objects.filter(group_id=group_id).select_related(
        "student__user", "group__teacher"
    )
